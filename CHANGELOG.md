...
- **Workbench UX pass.** Merged the overlapping *Hosts* and *Targets* tabs into one **Hosts** view (scope stats + coverage/ownership filters + progress/findings/owner/labels), reordered tabs to Dashboard · Hosts · Findings · Act · Credentials, collapsed the scan controls into a **▶ Scan** popover, and added a compact-density toggle, sticky table headers (usable at 100+ hosts), keyboard shortcuts (Esc closes, `/` focuses search), a first-run empty state, and a branded loading screen.
- **Web workbench (`recce serve`).** Drive the whole engagement from a browser — one recce instance hosts a FastAPI + React app (`recce serve -o eng`, default `http://<box>:8008`; `--host`/`--port`), the team opens the URL over the LAN. It serves the same datastore the CLI writes, so the two stay in sync. **Run scans from the UI** with live progress streamed over SSE (`enum`/`vulns`/`run`); a **Dashboard** with a *Next moves* panel; **Hosts**/**Targets** with a per-host detail drawer; **Findings** tiered by confidence (leads hidden by default, expand inline); an **Act** tab (ranked action cards, archetype filter, projected attack-path graph, one-click run of read-only/reversible actions); a **Loot** tab (looted credentials + a lockout-safe **spray** across a target scope that folds validated logins back in); one-click **report export** (Excel/HTML/Markdown/CSV); shared notes + review ticks for multi-tester collaboration; light mode by default with a dark toggle. Needs `fastapi`+`uvicorn` (bundled in the airgap package; dev install `pip install 'recce[bundle]'`). New `recce/webui/` (`app.py`, `jobs.py`, the built React frontend). The web UI is built into the burn package by `make_package`, and `tools/build_bundle.sh` freezes it (with an **offline build path** that reuses on-box deps when PyPI is unreachable).
- **API enumeration (`recce api`).** Discovers the API surface on web services enum found: **OpenAPI/Swagger specs** (which map the whole attack surface — paths, methods, sometimes example creds), **interactive API docs** (Swagger UI / ReDoc / GraphiQL), and **GraphQL introspection** (schema disclosure). Read-only GETs + one GraphQL introspection POST; findings flow through the normal pipeline (QoD / dedup / KEV/EPSS / tiering) and it's included in `recce run`'s deep sweep. New `recce/api.py`.
- **Data-driven detection: custom rules + negative matchers (Stage 6).** Extend or override recce's version->CVE detections as **data**, no code: `recce run|enum|vulns|scan --rules FILE` loads extra signatures from a JSON file (the airgap-friendly, stdlib-only form of a Nuclei template). Rules support a **negative matcher** (`absent`) — a check fires only when the banner does NOT contain given terms (decoys, distro-backported builds) — the main data-driven false-positive reducer. Loaded rules flow through the whole pipeline (QoD, backport downgrade, dedup, refutation, KEV/EPSS, tiering); a malformed rule is skipped, never fatal. See `docs/DETECTION-RULES.md`.
- **EPSS exploitation probability (Stage 5b) — prioritization is complete.** Each finding now carries its **EPSS** 30-day exploitation probability, shown in the **Fix first** column (`EPSS 97%`) when notable and folded into the sort. Findings now rank on all three axes: **KEV** (confirmed exploited) first, then **EPSS** (likely to be attacked), then **severity** (how bad) × **QoD** (how sure). Offline snapshot in `recce/epss.py`, refreshed from the EPSS feed at package build.
...
### Fixed
- **Concurrent workbench writes no longer clobber each other.** Collaboration state (assignments, labels, per-port status, dismissals, the activity feed, and chat) is stored as read-modify-write JSON, and the API endpoints run in a threadpool — so two people claiming/chatting at the same moment could lose an update (e.g. a dropped chat message). Every mutation is now serialized by a process-wide lock (`recce serve` is single-process); a concurrency test fires 40 simultaneous writers and asserts none are lost.
