...
| **.74** | telnet-target | Telnet :23 | *P10, profile `core`.* busybox telnetd with `root:toor` login. recce `telnet` — `telnet_present`, `telnet_iac_fingerprint`, `telnet_no_encrypt`, banner disclosure. |
| **.75** | tftp-target | TFTP :69/udp | *P10, profile `core`.* tftpd-hpa serving a bogus `running-config` + `test.txt`. recce `tftp` — `tftp_readable` (critical: canonical config filename), `tftp_open`. |
