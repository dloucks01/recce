...
| Nothing to `pip install`? | intentional | recce is **stdlib-only** at runtime. There are no Python deps — only the *system* tools below. `SYSTEM-REQUIREMENTS.txt` documents them. |
