...
_COMMANDS: dict = {
    ...
    "rservices":       _cmd("r-services (rlogin / rsh / rexec) — legacy cleartext",
                            "Services", "optional"),
    # --- T5+ scanner-expansion services (Phase 7a — GUI surface only) ---
    # Grouped by domain so the ScanTab card grid stays readable; each maps
    # to recce/services/<slug>.py via _service_helpers._MODULE_PATH.
    "ssh":             _cmd("SSH (banner + auth methods + weak-cred sweep)",
                            "Services", "optional", creds=True),
    "telnet":          _cmd("Telnet (banner + auth prompt + weak-cred sweep)",
                            "Services", "optional", creds=True),
    "mqtt":            _cmd("MQTT (unauth SUBSCRIBE + retained-topic dump)",
                            "Services", "optional", creds=True),
    "rtsp":            _cmd("RTSP (DESCRIBE fingerprint + weak-cred sweep on cameras)",
                            "Services", "optional", creds=True),
    "vault":           _cmd("HashiCorp Vault (seal status + auth mounts + KV probe)",
                            "Services", "optional", creds=True),
    "jenkins-jnlp":    _cmd("Jenkins JNLP (agent port — CLI RCE reachability)",
                            "Services", "optional"),
    "cups_lpd":        _cmd("CUPS LPD 515/tcp (LPR queue enum)", "Services", "optional"),
    "slp":             _cmd("SLP (Service Location Protocol — amplification + service directory)",
                            "Services", "optional"),
    "bgp":             _cmd("BGP (OPEN handshake — ASN + router-ID + capabilities)",
                            "Services", "optional"),
    "stun":            _cmd("STUN / TURN (public-IP disclosure + relay-allocation abuse)",
                            "Services", "optional", creds=True),
    "xmpp":            _cmd("XMPP (stream:features — TLS/SASL posture + in-band reg)",
                            "Services", "optional", creds=True),
    "guacamole":       _cmd("Apache Guacamole (auth surface + default-cred sweep)",
                            "Services", "optional", creds=True),
    "minecraft":       _cmd("Minecraft (server-list ping — MOTD + version + player list)",
                            "Services", "optional"),
    "nisyp":           _cmd("NIS / YP (ypserv — domain enum + passwd map extraction)",
                            "Services", "optional"),
    # --- Mail ---
    "imap":            _cmd("IMAP (CAPABILITY + STARTTLS posture + weak-cred sweep)",
                            "Mail", "optional", creds=True),
    "pop3":            _cmd("POP3 (CAPA + STARTTLS posture + weak-cred sweep)",
                            "Mail", "optional", creds=True),
    # --- Storage ---
    "iscsi":           _cmd("iSCSI (target discovery + CHAP posture)",
                            "Storage", "optional", creds=True),
    "nbd_ndmp":        _cmd("NBD / NDMP (export list + auth posture)",
                            "Storage", "optional"),
    "webdav":          _cmd("WebDAV (methods + PROPFIND + default-cred sweep)",
                            "Storage", "optional", creds=True),
    # --- Virtualization ---
    "vsphere":         _cmd("vCenter / ESXi vSphere (build + SDK reachability + auth)",
                            "Virtualization", "optional", creds=True),
    "cloud_metadata":  _cmd("Cloud metadata (SSRF-reachable IMDS / metadata endpoints)",
                            "Virtualization", "optional"),
    # --- Monitoring ---
    "nrpe":            _cmd("NRPE (Nagios remote plugin — command enum + CVE-2013-1362)",
                            "Monitoring", "optional"),
    "zabbix":          _cmd("Zabbix agent + server (10050/10051 — item probe + auth)",
                            "Monitoring", "optional", creds=True),
    # --- OT / ICS ---
    "s7":              _cmd("Siemens S7comm (CPU identity + module list)",
                            "OT/ICS", "optional"),
    "bacnet":          _cmd("BACnet/IP (Who-Is + device object + vendor)",
                            "OT/ICS", "optional"),
    "opcua":           _cmd("OPC-UA (GetEndpoints + security modes + anon posture)",
                            "OT/ICS", "optional"),
    "dnp3":            _cmd("DNP3 (link-layer + outstation identity)",
                            "OT/ICS", "optional"),
    "iec104":          _cmd("IEC-104 (station interrogation — SCADA telemetry)",
                            "OT/ICS", "optional"),
    "enip":            _cmd("EtherNet/IP (List Identity — Rockwell / Allen-Bradley PLC)",
                            "OT/ICS", "optional"),
    "coap":            _cmd("CoAP (5683/udp — resource discovery + amplification)",
                            "OT/ICS", "optional"),
    # --- Loot & Attack tier ---
