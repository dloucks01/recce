...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    _add_t4("gitlab",          "GitLab CE/EE HTTP probe (80/443/8080/8443): version + public projects + health + version-gated CVE-2021-22205/CVE-2023-2825 markers")
    _add_t4("grafana",          "Grafana dashboards probe (3000): /api/health fingerprint + version + SAFE single-shot admin:admin marker on /api/orgs + version-gated CVE-2021-43798/CVE-2024-9264 windows")
    _add_t4("jupyterhub",      "Jupyter Server / JupyterHub probe (8888/8000/8443): /api version + /hub/api/info + GET-only checks of /api/kernels (RCE oracle) and /api/contents (notebook filesystem)")
    _add_t4("minio",           "MinIO S3 object-store probe (9000/9001): /minio/health/live fingerprint, anonymous bucket listing on GET /, SAFE single-shot minioadmin/minioadmin AWS4 marker, fingerprint-gated CVE-2023-28432 env leak")
    _add_t4("ollama",          "Ollama local LLM daemon probe (11434): /api/version fingerprint + /api/tags model inventory + SAFE unauth POST /api/generate probe with an invalid sentinel model + version-gated CVE-2024-37032 marker")
